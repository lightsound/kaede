...
| (c) 髪 girl | ロングツインテール($0.083) | **成立(条件つき)**。スタイル忠実(チェリーピン保持)。首下に垂れる髪は「首上=全置換+首下=ベース差分(髪のみ)」の抽出で体の前面レイヤーに載る。**限界 2 つを記録**: 髪が硬直(揺れなし)・腕スイングが髪の後ろに隠れる(前面固定 z) — ゲームスケール(実表示 1/6)では許容と検収 |
