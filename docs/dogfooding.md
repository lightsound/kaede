...
| Phase 4（ビデオ通話 — 移行マイルストーン） | ✅ 完了（2026-08-07）— 通話・画面共有・録画・一覧/DL をオーナーが本番で実測確認。本番は増分⑥の procedure 構成（`call_config` 播種済み・Worker `kaede-call` 削除済み）。SpacetimeDB は CLI/ホスト/SDK とも 2.10.1 ピン |
