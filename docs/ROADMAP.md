...
- **⑦オブジェクトシステム（決定変更を了承・段階導入 — オーナー
  ...
  （コード定義か管理者設置の DB 行か — egress と⑥の編集 UI の流用可否に
  直結）は着手時の設計議論で確定する
