...
## Phase 4.5: 常用化 — ドッグフーディング実測フィードバック対応
...
- **④パネルレイアウトの整理** ⏸️ **後回し（2026-08-07 オーナー裁定）**:
  ...
  まとめて行う**
- **⑤通話接続の体感速度（調査→短縮）**: 「📞 通話に参加」→「📞 通話に
  接続中…」の待ちの内訳は ⑴ `join_group_call` procedure（Maincloud RTT ＋
  RealtimeKit API の直列呼び出し — meeting 作成は行が無い初回のみ
  0.5〜0.9s・参加トークン発行 0.6〜0.9s、Phase 4 増分⑥スパイク実測）
  ⑵ SDK init ＋ WebRTC join（ICE/DTLS ハンドシェイク — プロバイダ依存）
  ⑶ 接続後に始まる InCallPanel 遅延チャンク（UI Kit ＋ hls.js 等）の
  ダウンロード。まず本番でフェーズ分解を実測し、効く順に対策する:
  チャンクの先読み（通話ドック表示時に import() を発火 — ⑶を待ち時間から
  除去。lazy 化の目的「idle プレイヤーに配らない」はドック表示ゲートで
  維持される）・進捗の段階表示（単一の「接続中…」をやめる）・meeting の
  事前プロビジョニング（⑴の初回分 — コスト増と複雑化を伴うため実測後に
  判断）。⑵ は短縮不能な床の見込みなので、実測値を記録して「技術的に
  ここまで」の結論を出す
