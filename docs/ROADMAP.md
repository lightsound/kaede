...
- **既決方針と緊張関係があり設計議論待ち（実装前にオーナー合意が必須）**:
  クリック移動・会議室ゾーンの D&D 配置・オブジェクトシステム構想（下記）
...
**設計議論待ち（実装しない — オーナーと設計方針を合意してから増分化）**:

- **クリック移動（要望の根っこは「移動が面倒」）**: 既決との整合が必要 —
  「物理は楽しさの源泉として残す。移動の摩擦はポータル/ワープ等の
  ショートカットで緩和する」（VISION 体験の核 1）＋ サーバー権威ネットコード
  （クライアントは入力のみ送信）。候補: (a) **行き先ワープの拡充**（人・
  ...
  実装大） (c) 両方の段階導入。解はクリック移動に限らない前提で議論する
- **会議室ゾーンの D&D 配置（管理者の編集体験）**: 「管理者向けマップ
  エディタは SaaS 化フェーズに後置」（VISION）との線引き、および増分②の
  「設置座標はクライアントから受けない」（管理者が現地へ歩いて設置する
  方式）の変更判断が必要。論点: ゾーンは既にランタイム行（コード定義の
  マップジオメトリとは別物）なので「ゾーン矩形のドラッグ編集」をマップ
  エディタ本体と別枠に整理できるか。サーバー側は admin 検証＋マップ境界
  クランプで座標受理自体は安全にできる（位置の信頼問題は管理者限定の
  設置操作には及ばない）
- **オブジェクトシステム構想（床・物理設定・固有の振る舞い・ゾーンの
  オブジェクト化）**: マップスキーマ・決定論物理（衝突するオブジェクトは
  予測/リプレイに載る）・描画レイヤー（キャラより前面/背面）・購読
  （配置を DB 行にするなら egress）への影響が大きい。段階分けの叩き台:
  ①装飾のみ（コード定義・描画のみ・前後固定） → ②前後関係と衝突の設定
  （物理拡張 — ジャンプで越える障害物） → ③インタラクション（「座る」等 —
  Phase 5 のジェスチャーと連動） → ④会議室ゾーンの見た目をオブジェクトに
  寄せる（「少し広い電話ボックス」）。設置を誰がどう行うか（コード定義か
  管理者設置か）は D&D 論点と合流するため、セットで議論する

**完了条件**: 増分①〜⑤がマージされ、オーナーとコミュニティの日常利用で
「見づらい・気になる・遅い」の指摘が解消されている。設計議論待ちの 3 項目は
方針合意（実装する/しない・どの形で）まで — 実装自体は合意後の増分。
