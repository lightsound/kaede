...
**合意済みの後続増分（オーナー裁定 2026-08-07。着手は⑤の後、**⑦→⑥→⑧**の順
— 2026-08-07 に⑥⑦の順序を入れ替え、③④は上記のとおりリデザイン段階へ後置）**:
いずれも**着手直前に設計議論のセッションを置き、設計合意 → 実装増分化**の
順で進める。今すぐ設計を確定させない理由: 残る増分⑤はどの決定にも依存せず、
本番の実測データが揃ってから設計した方が精度が高い。
**順序入れ替えの理由（オーナー 2026-08-07）**: 会議室ゾーンは⑦で
オブジェクトへ寄せる方向（⑦の段階④）のため、ゾーン専用の D&D 編集 UI を
先に作ると⑦の設置/編集機構で確実に作り直しになる（二度手間）。⑦の編集
機構を先に設計し、⑥は「ゾーンがその機構に乗る」形へ縮める（⑦④に吸収され
消える可能性も含めて⑦の設計議論で確定する）。

- **⑦オブジェクトシステム（決定変更を了承・段階導入 — オーナー
  2026-08-07）**: マップにオブジェクトを配置できるようにする（パソコン
  デスク＝「座る」アクション、ぬいぐるみ＝装飾のみ、のようにオブジェクト
  ごとに固有の振る舞い）。段階の叩き台: ①装飾のみ（描画のみ・前後固定） →
  ②前後関係と衝突の設定（物理拡張 — ジャンプで越える障害物。衝突する
  オブジェクトは決定論物理・予測/リプレイに載る） → ③インタラクション
  （「座る」等 — Phase 5 のジェスチャーと連動） → ④会議室ゾーンの見た目を
  オブジェクトに寄せる（「少し広い電話ボックス」）。「床」概念のマップ
  スキーマへの導入・描画レイヤー（キャラより前面/背面）・設置の方式
  （コード定義か管理者設置の DB 行か — egress と編集 UI の設計に直結）は
  着手時の設計議論で確定する。**設計議論の第一論点は「会議室ゾーンと
  オブジェクトの関係」**: 現記載の「見た目を寄せる」（④）に留めるか、
  ゾーン自体をオブジェクトの一種へ統合するか — 統合の深さでチャット可視性
  RLS・在室判定・group_call 連動の扱いが大きく変わるため、最初に確定させる
  （「会議室はオブジェクトにするべき」というオーナーフィードバックの受け皿）
- **⑥会議室ゾーンの D&D 配置（決定変更を了承 — オーナー 2026-08-07。
  同日の順序入れ替えで⑦の後へ後置）**:
  「管理者向けマップエディタは SaaS 化フェーズに後置」（VISION）の対象を
  **マップジオメトリ（足場・ロープ等）のエディタ**に限定する線引きへ変更し、
  ランタイム行であるゾーンの矩形編集 UI（ドラッグ移動・リサイズ）を作る。
  増分②の「設置座標はクライアントから受けない」規則も**管理者の
  設置操作に限り**変更する（サーバー側は admin 検証＋マップ境界クランプで
  受理 — 位置の信頼問題は管理者限定の設置操作には及ばない）。**実装は⑦の
  設置/編集機構に相乗りする**（ゾーン専用の編集 UI は作らない — 上の
  順序入れ替えの理由）。⑦の設計次第で本増分は⑦④に吸収され独立増分として
  消え得る
