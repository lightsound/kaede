...
- **②ゲーム画面の黒余白の解消** ✅ **実装済み（2026-08-07）**: 固定
  ...
  （増分③④のパネル整理はこの配置が前提）
- **③初回ロードのレイアウトシフト解消**: 現状は「サーバーに接続中…」だけが
  出て、接続完了後にチャットパネル等が一斉にポップインする。パネルの骨格
  （スケルトン/プレースホルダ）を接続前から出す・出現順を設計する等、
  ローディングの見せ方を作る（④の配置見直しと整合させる）
- **④パネルレイアウトの整理**: 常設パネルが画面の四隅＋左端スタックに
  7 箇所散っており（左下: 名前・ステータス・立ち話・通話 / 右下: チャット /
  左上: 録画一覧 / 右上: 認証・管理）、ゲーム画面を覆って見づらい。
  表示/非表示の切替（最小化・タブ化等）と配置の見直しで「デフォルトは
  ワールドが見える」状態にする
...
順で進める。今すぐ設計を確定させない理由: ①〜⑤はどの決定にも依存せず、
パネル整理後の UI 状態・本番の実測データ・⑥の編集機構の実装経験が揃って
から設計した方が精度が高い。
...
**完了条件**: 増分①〜⑤がマージされ、オーナーとコミュニティの日常利用で
「見づらい・気になる・遅い」の指摘が解消されている（oVice 解約判断は
ここで可能）。⑥〜⑧は常用のブロッカーではない拡張のため完了条件に
