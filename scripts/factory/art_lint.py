...
# owner-approved amplitude (the wan-era boy sheet's p-p 3px at the 96px
# sheet scale). This is the gate that makes NG1/NG2 shapes unshippable:
# both rejected fix attempts (PR #108: 46/46/44/49, PR #109: 44/48/47/44)
# fail it, as do the three rejected committed families (girl 47/44/45/48,
# carry 45/51/44/51, light 46/48/47/47). Sheets composed before the
# prescription (the one-bump boy base, 46/49/47/47) fail on re-import by
# design: the fix is re-running the walk lane, not exempting the sheet.
BOB_PP_RANGE = (2, 5)
BOB_CONTRAST_MIN = 1.0
...
# measurement the way lint.neckFrom validates necks.
HAND_SKIN_RADIUS = 3
